...
class SearchWP_Live_Search_Settings {
	...
	private function get_settings_fields() {
		...
		$fields   = [];
		$settings = searchwp_live_search_bio()
            ->get( 'Settings_Api' )
		...
		<form class="searchwp-admin-settings-form" method="post">
            ...
            <p class="submit">
                <button type="submit" class="searchwp-btn searchwp-btn-md searchwp-btn-accent" name="searchwp-live-search-settings-submit">
	                <?php esc_html_e( 'Save Settings', BIO_PLUGIN_NAME ); ?>
                </button>
            </p>
		</form>
		<?php
	...
	private function output_field_checkbox( $args ) {
		...
		$value   = searchwp_live_search_bio()->get( 'Settings_Api' )->get( $args['slug'], $default );
	...
	private function output_field_select( $args ) {
		...
		$default = isset( $args['default'] ) ? esc_html( $args['default'] ) : '';
		$slug    = sanitize_key( $args['slug'] );
		$value   = searchwp_live_search_bio()->get( 'Settings_Api' )->get( $slug, $default );
                ...
                <ul>
                    <li class="searchwp-settings-nav-tab-wrapper searchwp-settings-nav-tab-active postbox-wrapper searchwp-settings-nav-tab-searchwp-live-search-wrapper">
                        <a href="https://searchwp-plugin.local/wp-admin/admin.php?page=searchwp-live-search" class="searchwp-settings-nav-tab searchwp-settings-nav-tab-active postbox searchwp-settings-nav-tab-searchwp-live-search">
                            <span><?php esc_html_e( 'Settings', BIO_PLUGIN_NAME ); ?></span>
                        </a>
                    </li>
                </ul>
            </nav>
	...
	public function page_searchwp_disabled() {
        ...
        <div class="searchwp-admin-wrap wrap">
            <div class="searchwp-settings-view">
				<?php searchwp_live_search_bio()->get( 'Settings' )->output(); ?>
            </div>
        </div>
		<?php
	}
    ...
    private function output_after_settings() {
        ...
        <div class="searchwp-settings-cta">
            <h5><?php esc_html_e( 'Get SearchWP Pro and Unlock all the Powerful Features', BIO_PLUGIN_NAME ); ?></h5>
            <p><?php esc_html_e( 'Thank you for being a loyal SearchWP Live Ajax Search user. Upgrade to SearchWP Pro to unlock all the powerful features and experience why SearchWP is the best WordPress search plugin.', BIO_PLUGIN_NAME ); ?></p>
            <p>
			    <?php
			    printf(
				    wp_kses( /* translators: %s - star icons. */
					    esc_html__( 'We know that you will truly love SearchWP Pro. It’s used on over 30,000 smart WordPress websites and is consistently rated 5-stars (%s) by our customers.', BIO_PLUGIN_NAME ),
					    [
						    'i' => [
							    'class'       => [],
							    'aria-hidden' => [],
            ...
            <h6><?php esc_html_e( 'Pro Features:', BIO_PLUGIN_NAME ); ?></h6>
            <div class="list">
                <ul>
                    <li><?php esc_html_e( 'Search all custom field data', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Make ecommerce metadata discoverable in search results', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Search PDF, .doc, .txt and other static documents', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Search custom database tables and other custom content', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Make your media library (images, videos, etc.) searchable', BIO_PLUGIN_NAME ); ?></li>
                </ul>
                <ul>
                    <li><?php esc_html_e( 'Search categories, tags and even custom taxonomies', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Easy integration with all WordPress themes and page builders', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Advanced search metrics and insights on visitor activity', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'Multiple custom search engines for different types of content', BIO_PLUGIN_NAME ); ?></li>
                    <li><?php esc_html_e( 'WooCommerce & Easy Digital Downloads support', BIO_PLUGIN_NAME ); ?></li>
                </ul>
            </div>
            <p><a href="https://searchwp.com/?utm_source=WordPress&utm_medium=Settings+Upgrade+Bottom+Link&utm_campaign=Live+Ajax+Search&utm_content=Get+SearchWP+Pro+Today+and+Unlock+all+the+Powerful+Features" target="_blank" rel="noopener noreferrer" title="<?php esc_html_e( 'Get SearchWP Pro Today', BIO_PLUGIN_NAME ); ?>"><?php esc_html_e( 'Get SearchWP Pro Today and Unlock all the Powerful Features', BIO_PLUGIN_NAME ); ?> &raquo;</a></p>
            <p>
	            <?php
	            echo wp_kses(
		            __( '<strong>Bonus:</strong> SearchWP Live Ajax Search users get <span class="green">50% off the regular price</span>, automatically applied at checkout!', BIO_PLUGIN_NAME ),
		            [
			            'strong' => [],
			            'span'   => [
				            'class' => [],
	...
	public function admin_footer_rate_us_searchwp_disabled( $text ) {
		...
		$url = 'https://wordpress.org/support/plugin/BIO_PLUGIN_NAME/reviews/?filter=5#new-post';

		return sprintf(
			wp_kses( /* translators: $1$s - SearchWP plugin name; $2$s - WP.org review link; $3$s - WP.org review link. */
				__( 'Please rate %1$s <a href="%2$s" target="_blank" rel="noopener noreferrer">&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="%3$s" target="_blank" rel="noopener">WordPress.org</a> to help us spread the word. Thank you from the SearchWP team!', BIO_PLUGIN_NAME ),
				[
					'a' => [
						'href'   => [],
						'target' => [],
	...
	public static function get_dyk_block_output() {
        ...
        <div class="searchwp-settings-dyk">
            <h5><?php esc_html_e( 'Did You Know?', BIO_PLUGIN_NAME ); ?></h5>
            <p>
	            <?php
	            echo wp_kses(
		            __( 'By default, WordPress doesn’t make all your content searchable. <strong><em>That’s frustrating</em></strong>, because it leaves your visitors unable to find what they are looking for!', BIO_PLUGIN_NAME ),
		            [
			            'strong' => [],
			            'em'     => [],
		            ]
            ...
            <p><?php esc_html_e( 'With SearchWP Pro, you can overcome this obstacle and deliver the best, most relevant search results based on all your content, such as custom fields, ecommerce data, categories, PDF documents, rich media and more!', BIO_PLUGIN_NAME ); ?></p>
            <p><a href="https://searchwp.com/?utm_source=WordPress&utm_medium=Settings+Did+You+Know+Upgrade+Link&utm_campaign=Live+Ajax+Search&utm_content=Get+SearchWP+Pro+Today" target="_blank" rel="noopener noreferrer" title="<?php esc_html_e( 'Get SearchWP Pro Today', BIO_PLUGIN_NAME ); ?>"><?php esc_html_e( 'Get SearchWP Pro Today', BIO_PLUGIN_NAME ); ?> &raquo;</a></p>
            <p>
	            <?php
	            echo wp_kses(
		            __( '<strong>Bonus:</strong> SearchWP Live Ajax Search users get <span class="green">50% off the regular price</span>, automatically applied at checkout!', BIO_PLUGIN_NAME ),
