...
class SearchWP_Live_Search_Settings_Api {
	...
	private function save_settings() {
		...
		SearchWP_Live_Search_Notice::success( esc_html__( 'Settings were successfully saved.', BIO_PLUGIN_NAME ) );
	}
	...
	public function get_registered_settings() {
		...
		$defaults = [
			'general-heading'         => [
				...
				'content' => '<h3>' . esc_html__( 'General', BIO_PLUGIN_NAME ) . '</h3>',
				'type'    => 'content',
				'class'   => [ 'section-heading' ],
			],
			'enable-live-search'      => [
				'slug'       => 'enable-live-search',
				'name'       => esc_html__( 'Enable Live Search', BIO_PLUGIN_NAME ),
				'desc'       => esc_html__( 'Check this option to automatically enhance your search forms with Live Ajax Search.', BIO_PLUGIN_NAME ),
				'type'       => 'checkbox',
				'default'    => true,
			...
			'results-heading'         => [
				...
				'content' => '<h3>' . esc_html__( 'Results', BIO_PLUGIN_NAME ) . '</h3>',
				'type'    => 'content',
				'class'   => [ 'section-heading' ],
			],
			'include-frontend-css'    => [
				'slug'    => 'include-frontend-css',
				'name'    => esc_html__( 'Include Styling', BIO_PLUGIN_NAME ),
				'desc'    => esc_html__( 'Determines which CSS files to load and use for the site. "Positioning and visual styling" is recommended, unless you are experienced with CSS or instructed by support to change settings.', BIO_PLUGIN_NAME ),
				'type'    => 'select',
				'default' => 'all',
				'options' => [
					'all'      => esc_html__( 'Positioning and visual styling', BIO_PLUGIN_NAME ),
					'position' => esc_html__( 'Positioning styling only', BIO_PLUGIN_NAME ),
					'none'     => esc_html__( 'No styling', BIO_PLUGIN_NAME ),
				],
			],
			'results-pane-position'   => [
				'slug'    => 'results-pane-position',
				'name'    => esc_html__( 'Positioning', BIO_PLUGIN_NAME ),
				'desc'    => esc_html__( 'Selects where to position the results pane relative to the search form.', BIO_PLUGIN_NAME ),
				'type'    => 'select',
				'default' => 'bottom',
				'options' => [
					'bottom' => esc_html__( 'Below the search form', BIO_PLUGIN_NAME ),
					'top'    => esc_html__( 'Above the search form', BIO_PLUGIN_NAME ),
				],
			],
			'results-pane-auto-width' => [
				'slug'    => 'results-pane-auto-width',
				'name'    => esc_html__( 'Auto Width', BIO_PLUGIN_NAME ),
				'desc'    => esc_html__( 'Check this option to align the results pane width with the search form width.', BIO_PLUGIN_NAME ),
				'type'    => 'checkbox',
				'default' => true,
			],
		];
