...
class SearchWP_Live_Search {
	...
	private function load_textdomain() {
		...
		load_plugin_textdomain( BIO_PLUGIN_NAME, false, dirname( plugin_basename( SEARCHWP_LIVE_SEARCH_PLUGIN_FILE ) ) . '/languages/' );
	}
		...
		$message =
			sprintf(
				wp_kses( /* translators: %1$s - SearchWP.com URL, %2$s - SearchWP.com URL. */
					__( '<strong>SearchWP Live Ajax Search</strong><br><a href="%1$s" target="_blank" rel="noopener noreferrer">Personalize your search results</a> and discover what your visitors are searching for at the same time with <a href="%2$s" target="_blank" rel="noopener noreferrer">SearchWP Pro!</a>', BIO_PLUGIN_NAME ),
					[
						'a'      => [
							'href'   => [],
							'target' => [],
	...
	public function settings_link( $links, $plugin_file, $plugin_data, $context ) {
		...
		if ( ! SearchWP_Live_Search_Utils::is_searchwp_active() ) {
			$custom['pro'] = sprintf(
				...
				esc_attr__( 'Upgrade to SearchWP Pro', BIO_PLUGIN_NAME ),
				esc_html__( 'Get SearchWP Pro', BIO_PLUGIN_NAME )
			);
		}
		...
		$custom['settings'] = sprintf(
			...
			esc_attr__( 'Go to SearchWP Settings page', BIO_PLUGIN_NAME ),
			esc_html__( 'تنظیمات', BIO_PLUGIN_NAME )
		);
		...
		$custom['docs'] = sprintf(
			...
			esc_attr__( 'Read the documentation', BIO_PLUGIN_NAME ),
			esc_html__( 'Docs', BIO_PLUGIN_NAME )
		);
