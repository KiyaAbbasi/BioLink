...
class SearchWP_Live_Search_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'searchwp_live_search_bio',
			esc_html__( 'SearchWP Live Search', BIO_PLUGIN_NAME ),
			[ 'description' => esc_html__( 'SearchWP Live Search', BIO_PLUGIN_NAME ) ]
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$placeholder = empty( $instance['placeholder'] ) ? esc_html__( 'Search for...', BIO_PLUGIN_NAME ) : $instance['placeholder'];
        ...
        <form role="search" method="get" class="searchwp-live-search-widget-search-form" action="<?php echo esc_url( $destination ); ?>">
            ...
            <label>
                <span class="screen-reader-text"><?php esc_html_e( 'Search for:', BIO_PLUGIN_NAME ); ?></span>
            ...
            <input type="submit" class="search-submit" value="<?php esc_html_e( 'Search', BIO_PLUGIN_NAME ); ?>">
	...
	private function form_get_v3_engines() {
		...
		$engines['default'] = esc_html__( 'Default', BIO_PLUGIN_NAME );
	...
	private function form_title_html( $instance ) {
		...
		$widget_title = isset( $instance['title'] ) ? $instance['title'] : esc_html__( 'Search', BIO_PLUGIN_NAME );

		?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', BIO_PLUGIN_NAME ); ?></label>
	...
	private function form_engines_html( $instance ) {
		...
		?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'engine' ) ); ?>"><?php esc_html_e( 'SearchWP Engine:', BIO_PLUGIN_NAME ); ?></label>
	...
	private function form_configs_html( $instance ) {
		...
		$form = searchwp_live_search_bio()->get( 'Form' );

		?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'config' ) ); ?>"><?php esc_html_e( 'Configuration:', BIO_PLUGIN_NAME ); ?></label>
	...
	private function form_advanced_html( $instance ) {
		...
		$widget_placeholder = isset( $instance['placeholder'] ) ? $instance['placeholder'] : esc_html__( 'Search for...', BIO_PLUGIN_NAME );
		$widget_destination = isset( $instance['destination'] ) ? $instance['destination'] : '';

		$swp_uniqid = uniqid( 'swp' );

		?>
        <p><a href="#" class="button" onclick="document.getElementById('searchwp-live-search-widget-advanced-<?php echo sanitize_key( $swp_uniqid ); ?>').style.display = 'block'; this.parentNode.style.display = 'none';"><?php esc_html_e( 'Advanced', BIO_PLUGIN_NAME ); ?></a></p>
        <div id="searchwp-live-search-widget-advanced-<?php echo sanitize_key( $swp_uniqid ); ?>" style="display:none;">
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'placeholder' ) ); ?>"><?php esc_html_e( 'Placeholder:', BIO_PLUGIN_NAME ); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'destination' ) ); ?>"><?php esc_html_e( 'Destination fallback URL (optional):', BIO_PLUGIN_NAME ); ?></label>
