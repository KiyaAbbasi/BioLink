...
class SearchWP_Live_Search_Form {
	...
	private function apply_settings() {
		...
		$settings_api = searchwp_live_search_bio()->get( 'Settings_Api' );
	...
	public function assets() {
		...
		wp_enqueue_style(
			...
			BIO_PLUGIN_URL . '/inc/ajax-search/assets/styles/style.css',
			null,
		...
		wp_register_script(
			...
			BIO_PLUGIN_URL . "/inc/ajax-search/assets/javascript/dist/script{$debug}.js",
			[ 'jquery' ],
			SEARCHWP_LIVE_SEARCH_VERSION,
			true
		);
		...
		if ( apply_filters( 'searchwp_live_search_direct_search', false ) ) {
			$ajaxurl = BIO_PLUGIN_URL . '/inc/ajax-search/direct.php';
		}
		...
		$params = [
			...
			'config'              => $this->configs,
			'msg_no_config_found' => esc_html__( 'No valid SearchWP Live Search configuration found!', BIO_PLUGIN_NAME ),
			'aria_instructions'   => esc_html__( 'When autocomplete results are available use up and down arrows to review and enter to go to the desired page. Touch device users, explore by touch or with swipe gestures.' , BIO_PLUGIN_NAME ),
		];
	...
	public function base_styles() {
        ...
        <style>
            ...
            .searchwp-live-search-no-min-chars:after {
                content: "<?php esc_attr_e( 'Continue typing', BIO_PLUGIN_NAME ); ?>";
                display: block;
                text-align: center;
                padding: 2em 2em 0;
            }
